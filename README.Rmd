...
### Geocoding
Along with its cousin [`postmastr`](https:://slu-openGIS.github.io/postmastr/), `gateway` can be used to gecode data. In order for the local geocoder to work correctly, data needed to be parsed and standardized ahead of time:
...
### Spatial Joins
The `gateway` package contains data for further modifying point data from St. Louis. For instance, we could convert these to a shapefile representing the neighborhood polygons where sushi resturants are located:
